...
module.exports.getPurchaseData = function (purchase, options) {
	...
	data.push({
		bundleId: getSubscriptionBundleId(purchase.receipt),
...
function getSubscriptionBundleId(data) {
	if (data[REC_KEYS.BUNDLE_ID]) {
		return data[REC_KEYS.BUNDLE_ID];
	}
	if (data[REC_KEYS.BID]) {
		return data[REC_KEYS.BID];
	}
	return null;
}
